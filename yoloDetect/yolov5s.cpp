...
Yolov5s::Yolov5s(Configuration config)
{
...
void Yolov5s::detect_v5(Mat &frame)
{
    ...
    // 开始推理
    vector<Value> ort_outputs = ort_session->Run(RunOptions{ nullptr }, inputNamesCStr.data(), &input_tensor_, 1, outputNamesCStr.data(), output_names.size());   // 开始推理
    //generate proposals
...
//    std::cout << "Preds size: " << ort_outputs[0].GetTensorTypeAndShapeInfo().GetShape()[1] << std::endl;
//    const float* preds = ort_outputs[0].GetTensorMutableData<float>();
//    std::cout << "Preds first few values: ";
//    for (int i = 1721590; i < 1721675; i++) {
//        qDebug() << preds[i] << " ";
//    }
...
void Yolov5s::normalize_(Mat img)
{
...
Mat Yolov5s::resize_image(Mat srcimg, int *newh, int *neww, int *top, int *left)
{
...
void Yolov5s::nms(vector<BoxInfo> &input_boxes)
{
