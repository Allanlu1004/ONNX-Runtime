...
Yolo::Yolo()
{
//    Net_config yolo_nets = { 0.5, 0.6, 0.5 ,"/home/pi/Desktop/softward/code/code_yolo/yoloDetect/onnxmodels/v5lite-c.onnx", "/home/pi/Desktop/softward/code/code_yolo/yoloDetect/sample/coco.names" };
...
std::vector<BoxInfo> Yolo::detect(cv::Mat &frame)
{
    ...
    LogWrite::instance().addEntry("message","infer image success and get pread!");
...
bool Yolo::inityolo(std::string modelPath)
{
    Net_config yolo_nets = { 0.5, 0.6, 0.5 ,modelPath, "/home/pi/Desktop/softward/code/code_yolo/yoloDetect/sample/coco.names" };
    LogWrite::instance().addEntry("message","load Net config success!");
    // 初始化阈值
    this->confThreshold = yolo_nets.confThreshold;
    this->nmsThreshold = yolo_nets.nmsThreshold;
    this->objThreshold = yolo_nets.objThreshold;
    
    // 模型路径处理
    std::string model_path =yolo_nets.model_path;
    ...
    // 加载类别文件
    std::string classesFile = yolo_nets.classesFile;
